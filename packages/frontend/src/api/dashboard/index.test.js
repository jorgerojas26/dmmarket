...
describe('fetchDashboardSales', () => {
    ...
    it('construye URL sin compareFrom/compareTo', async () => {
        ...
        expect(global.fetch).toHaveBeenCalledWith('/api/dashboard/sales?from=2026-07-01&to=2026-07-27&showNoe=false');
    });

    it('incluye compareFrom y compareTo en la URL', async () => {
        global.fetch.mockResolvedValue({ ok: true, json: async () => ({}) });
        await fetchDashboardSales({
            from: '2026-07-01',
            to: '2026-07-27',
            showNoe: false,
    ...
    it('lanza error si la respuesta no es ok', async () => {
        ...
        await expect(fetchDashboardSales({ from: '2026-07-01', to: '2026-07-27', showNoe: false })).rejects.toThrow(
            'Dashboard API error: 500',
        );
    });
});
