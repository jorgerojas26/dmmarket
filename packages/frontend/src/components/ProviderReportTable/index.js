...
const ProviderReportTable = ({ data }) => {
    const maxMonto = useMemo(() => Math.max(...data.map((row) => row.monto || 0), 0), [data]);
                ...
                return (
                    <li key={`${row.proveedor}-${index}`} className="provider-report__item">
                        <div className="provider-report__row">
                            <span className={`provider-report__rank ${rankClass}`}>{index + 1}</span>
                            <div className="provider-report__info">
                                ...
                                <div className="provider-report__bar-bg">
                                    <div className="provider-report__bar" style={{ width: `${percentage}%` }} />
                                </div>
                            </div>
                            <span className="provider-report__amount">{formatCurrency(row.monto)}</span>
                        </div>
                    </li>
                );
            })}
