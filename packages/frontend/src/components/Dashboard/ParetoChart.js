...
const ParetoChart = ({ products = [], summary = null, loading = false }) => {
  ...
  const handlePrint = useCallback(() => {
    ...
    const docDef = buildParetoPdf(
      filteredProducts,
      labels[abcFilter] || "Todos los productos",
    );
    ...
    <div className="dashboard-panel" style={{ padding: "16px 20px" }}>
      ...
      >
        ...
        <span style={{ fontWeight: 400, color: "#9ca3af" }}>
          80% de la ganancia viene del 20% de productos
        </span>
      </div>
      ...
      >
        {summaryCards.map((card) => (
          ...
          >
            ...
            <div
              style={{ fontSize: 20, fontWeight: 700, color: "#e4e6ea" }}
            >
              {card.count}
            </div>
            <div style={{ fontSize: 12, color: card.accent }}>
              {card.pct}% de ganancia
            </div>
          </div>
        ))}
        <div
      ...
      <div style={{ height: 380, marginBottom: 12 }}>
        <ResponsiveBar
          ...
          colors={({ index }) => {
            const cp =
              chartData[index]?.cumulativePercent ?? 100;
            return cp <= 80
              ? "#22c55e"
              : cp <= 95
                ? "#f59e0b"
            ...
            style={{
              ...
              background:
                abcFilter === tab.key
                  ...
                  : "transparent",
              color:
                abcFilter === tab.key ? tab.color : "#9ca3af",
              fontSize: 12,
